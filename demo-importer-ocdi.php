...
class Abc_Ocdi_Demo_Importer_OCDI {
	...
	public function after_import( $selected_import ) {
		...
		update_option( 'dialer_ocdi_importer_rewrite_flash', true );
	}

	// Assign Menu update 
	private function assign_menu() {
		$primary  			= get_term_by( 'name', 'Primary menu', 'nav_menu' );
		$sidebar  		    = get_term_by( 'name', 'Sidebar Menu', 'nav_menu' );		
		set_theme_mod( 'nav_menu_locations', array(
			'menu-1'  	=> $primary->term_id,
			'menu-2'  	=> $sidebar->term_id,		
	...
	private function update_elementor_option() {
	    ...
	    update_option('_elementor_global_css', $elementor_global_css);   
	    update_option('dialer_elementor_custom_setting_imported', 'elementor_custom_setting_imported');
	}	
	 // rewrite
